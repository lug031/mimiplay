...
export function AdminSidebar() {
  ...
  return (
    <aside
      className={
        "flex w-full shrink-0 flex-col border-b border-white/10 bg-mimi-black text-white " +
        "md:fixed md:inset-y-0 md:left-0 md:z-40 md:w-60 md:overflow-hidden " +
    ...
    >
      ...
      <nav className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto overscroll-contain p-2 [scrollbar-gutter:stable]">
        {nav.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              [
                "shrink-0 rounded-mimi px-3 py-2.5 text-sm font-semibold transition",
